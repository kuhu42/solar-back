/*
  # Complete GreenSolar Backend Schema

  1. Authentication & Users
    - `auth.users` (Supabase managed)
    - `public.user_profiles` - Extended user data with roles and approvals
    - `public.user_sessions` - Track active sessions and OTP verification

  2. Core Business Tables
    - `projects` - Solar/wind installation projects
    - `tasks` - Installation/maintenance tasks
    - `leads` - Sales leads and conversions
    - `complaints` - Customer service tickets
    - `attendance` - Staff check-in/out with GPS
    - `inventory` - Equipment tracking with serials
    - `invoices` & `invoice_items` - Billing system
    - `commissions` - Freelancer earnings tracking
    - `notifications` - Real-time alerts

  3. File Management
    - `documents` - PDF certificates, contracts, reports
    - `photos` - Installation photos, complaint evidence
    - Storage buckets for organized file storage

  4. Analytics & Reporting
    - `analytics_events` - Track user actions and performance
    - `pipeline_stages` - Project progression tracking

  5. Security
    - Comprehensive RLS policies per role
...
CREATE TABLE IF NOT EXISTS commissions (
  ...
  project_id uuid REFERENCES projects(id),
  commission_type text NOT NULL, -- 'lead_conversion', 'project_completion', 'bonus'
  base_amount numeric NOT NULL,
  commission_rate numeric NOT NULL, -- percentage
...
CREATE TABLE IF NOT EXISTS documents (
  id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
  name text NOT NULL,
  type text NOT NULL, -- 'certificate', 'invoice', 'contract', 'photo', 'report'
...
DECLARE
  ...
  commission_rate_val numeric := 0.10; -- 10% default
...
BEGIN
  ...
  ELSIF commission_type_param = 'project_completion' AND project_uuid IS NOT NULL THEN
    ...
    commission_rate_val := 0.05; -- 5% for project completion
  END IF;
...
BEGIN
  IF NEW.status = 'completed' AND OLD.status != 'completed' AND NEW.assigned_to IS NOT NULL THEN
    -- Check if assigned user is freelancer
