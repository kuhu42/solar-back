...
const LoginScreen = () => {
  ...
  alert(`✅ ${file.name} ready to upload! (${(file.size / 1024).toFixed(2)} KB)`);
};
...
const handleCustomerSubmit = async (e) => {
  ...
  try {
    if (isLiveMode) {
      // Prepare documents object - only include files that were uploaded
      const documents = {};
      if (customerForm.customerPhoto) documents.customerPhoto = customerForm.customerPhoto;
      if (customerForm.signPhoto) documents.signPhoto = customerForm.signPhoto;
      if (customerForm.aadharPhoto) documents.aadharPhoto = customerForm.aadharPhoto;
      if (customerForm.panPhoto) documents.panPhoto = customerForm.panPhoto;
      if (customerForm.bankBookPhoto) documents.bankBookPhoto = customerForm.bankBookPhoto;
      ...
      const registrationData = {
        ...
        documents: documents // ✅ NEW: Pass documents to be uploaded
      };

      console.log('🔍 Registration data being sent to authService:', {
        ...registrationData,
        documents: Object.keys(documents) // Just log the keys, not the actual files
      });
      ...
      alert(`Customer registration successful for ${getCityFromPincode(customerForm.pincode)}! Your documents have been uploaded. Your default password is: ${DEFAULT_PASSWORD}. You can login immediately.`);
    } else {
