...
const CustomerDashboard = () => {
  const { currentUser, projects, complaints, invoices, inventory, users, dispatch, showToast } = useApp();
  ...
  project.customer_id === currentUser.id ||
  project.customerId === currentUser.id  // Legacy field support
);
  ...
  <span className="flex items-center">
    ...
    Started: {project.start_date || project.startDate || project.created_at?.split('T')[0]}
  </span>
  <span className="flex items-center">
  ...
  const agent = users.find(u => 
    u.id === project.agent_id || u.id === project.assigned_to
  );
  return agent ? (
    <div className="flex items-center space-x-2">
      ...
      <button className="flex items-center px-2 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm">
        <Mail className="w-4 h-4" />
      </button>
    </div>
  ) : (
    <button className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm" disabled>
      <Phone className="w-4 h-4 mr-2" />
      No Agent Assigned
    </button>
  );
  ...
  myProjects.some(project => 
    (project.serial_numbers || project.serialNumbers || []).includes(item.serialNumber || item.serial_number)
  )
).map((item) => (
  <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
    <div>
      <h5 className="font-medium text-gray-900">{item.serialNumber || item.serial_number}</h5>
      <p className="text-sm text-gray-600">{item.model} • {item.type.replace('_', ' ')}</p>
      {item.install_date && (
        <p className="text-xs text-gray-500">Installed: {item.install_date}</p>
      )}
    </div>
    <div className="text-right">
      ...
      <p className="text-xs text-gray-500 mt-1">
        Warranty: {item.warranty_expiry || item.warrantyExpiry}
      </p>
    </div>
  </div>
))}
